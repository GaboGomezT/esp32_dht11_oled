#ifndef _Laika_FirebaseESP32_CPP_INCLUDED
#define _Laika_FirebaseESP32_CPP_INCLUDED
...
void FirebaseESP32::begin(String host, String auth) {
	host.replace("https:", "");  // reemplaza  para evitar errores
	host.replace("/", ""); // reemplaza  para evitar errores
	_host = host;
	_auth = auth;
}
...
void FirebaseESP32::stream(String path, StreamHandlerCallback callback) {
	...
	xTaskCreatePinnedToCore([](void* param) {
		...
		vTaskDelete(NULL);
		FirebaseStreamTaskHandle = NULL;							// lo estamos dejando en esta prioridad
